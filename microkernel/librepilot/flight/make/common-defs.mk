...
# Set to YES to enable Minion
MINION               ?= NO

# Set to YES to compile with LLVM/Clang
LLVM                 ?= NO
...
ifeq ($(LLVM), YES)
    ARCHFLAGS := --target=$(patsubst %-,%,$(ARM_SDK_PREFIX)) $(ARCHFLAGS)
endif
...
ifeq ($(LLVM), YES)
    ...
    CFLAGS += -Xclang -disable-llvm-optzns
endif
CFLAGS += -Wall -Wextra
...
ifeq ($(LLVM), YES)
    ...
    CFLAGS += -Wno-error=parentheses-equality
    CFLAGS += -Wno-error=non-literal-null-conversion
else
...
ifeq ($(LLVM), YES)
    ASFLAGS := --target=$(patsubst %-,%,$(ARM_SDK_PREFIX)) $(ASFLAGS)
else
...
ifeq ($(LLVM), YES)
    ...
    LDFLAGS += --warn-common --fatal-warnings --sort-common --gc-sections
    LDFLAGS += -Map=$(OUTDIR)/$(TARGET).map --cref
