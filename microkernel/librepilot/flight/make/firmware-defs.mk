...
ifeq ($(LLVM), YES)
    ...
    CXX     = $(CCACHE) clang++
    AR      = llvm-ar rcs
    LD      = $(ARM_SDK_PREFIX)ld.gold -plugin=$(shell llvm-config --libdir)/LLVMgold.so
...
ifeq ($(LLVM), YES)
    MSG_BITCODE      = $(QUOTE) BC        $(MSG_EXTRA) $(QUOTE)
endif
...
$(OUTDIR)/$(notdir $(basename $(1))).o : $(1)
	...
	$(V1) $(CC) $(EMIT) -c $(THUMB) $$(CFLAGS) $$(CONLYFLAGS) $$(CPPFLAGS) $$< -o $$@
endef
...
$(OUTDIR)/$(notdir $(basename $(1))).o : $(1)
	...
	$(V1) $(CC) $(EMIT) -c $$(CFLAGS) $$(CONLYFLAGS) $$(CPPFLAGS) $$< -o $$@
endef
...
$(OUTDIR)/$(notdir $(basename $(1))).o : $(1)
	...
	$(V1) $(CXX) $(EMIT) -c $(THUMB) $$(CFLAGS) $$(CPPFLAGS) $$(CXXFLAGS) $$< -o $$@
endef
...
$(OUTDIR)/$(notdir $(basename $(1))).o : $(1)
	...
	$(V1) $(CPP) $(EMIT) -c $$(CFLAGS) $$(CPPFLAGS) $$(CXXFLAGS) $$< -o $$@
endef
...
$(1):  $(1).input_files
	@echo $(MSG_BITCODE) $$(call toprel, $$(patsubst %.elf,%.bc,$$@))
	$(V1) $(LD) -plugin-opt=emit-llvm $(THUMB) @$(1).input_files -o $$(patsubst %.elf,%.bc,$$@) $$(LDFLAGS)
	@echo $(MSG_LINKING) $$(call toprel, $$@)
	$(V1) $(LD) $(THUMB) @$(1).input_files -o $$(patsubst %.elf,%.bc,$$@) $$(LDFLAGS)
	#$(V1) $(CC) $(THUMB) $$(CFLAGS) $$(CPPFLAGS) $$(patsubst %.elf,%.bc,$$@) -o $$@
endef
