...
WARNFLAGS = -Werror -Wno-packed -Wno-error=unused-variable -Wno-error=reorder -Wno-error=float-equal -Wno-error=missing-declarations -Wno-error=unused-function
ifndef LLVM
WARNFLAGS += -Wno-error=double-promotion -Wno-psabi -Wno-error=pmf-conversions
endif
...
ifdef NOOPT
PX4_MAKE = $(v) GIT_SUBMODULES_ARE_EVIL=1 ARDUPILOT_BUILD=1 make -C $(SKETCHBOOK) -f $(PX4_ROOT)/Makefile EXTRADEFINES="$(SKETCHFLAGS) $(WARNFLAGS) "'$(EXTRAFLAGS)' APM_MODULE_DIR=$(SKETCHBOOK) SKETCHBOOK=$(SKETCHBOOK) CCACHE=$(CCACHE) PX4_ROOT=$(PX4_ROOT) NUTTX_SRC=$(NUTTX_SRC) MAXOPTIMIZATION="-O0" UAVCAN_DIR=$(UAVCAN_DIR)
PX4_MAKE_ARCHIVES = make -C $(PX4_ROOT) NUTTX_SRC=$(NUTTX_SRC) CCACHE=$(CCACHE) archives MAXOPTIMIZATION="-O0"
else
...
module_mk:
	...
	$(v) echo "EXTRACXXFLAGS = -Wframe-larger-than=1400" >> $(SKETCHBOOK)/module.mk.new
